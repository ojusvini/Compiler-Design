...
int main()
{
   ...
   terminal[c_term++] = '$';
    ...
    int dfa1[ns][c_term]; 
    int dfa2[ns][c_nterm];
    int dfa_error[ns][c_term];
    memset(dfa1,-1,sizeof(dfa1));
    memset(dfa2,-1,sizeof(dfa2));
	memset(dfa_error,-1,sizeof(dfa_error));
        ...
        while(k<c_term) {
            ...
            if(flag == 0) {
                if(i == 1 && k == c_term-1) {
                    ...
                    else if(terminal[k]=='i'|| terminal[k]=='(') {
                    	printf("e3\t");
                    	dfa_error[i][k] = 3;
					}
					else if(terminal[k]=='+'|| terminal[k]=='*'||terminal[k]=='$') {
						printf("e1\t");
                    	dfa_error[i][k] = 1;
					}
					else if(terminal[k]==')') {
    ...
    while(!(str[i]=='$'&&state.top()==1)) {
        ...
        if(!isupper(str[i]) && dfa1[cur_st][j]!=-1) {
        	...
        	if(dfa_error[state.top()][idx]==1) {
        		printf("ERROR: Missing Operand\n");
				state.push(3);	
				if(symbol.top()== '*' || symbol.top()== '+' ) {
					symbol.push('i');
				}
				else if(symbol.top()== 'i'){
					symbol.push('(');
				}
				flag = 1;
			}
			else if(dfa_error[state.top()][idx]==2) {
				printf("ERROR: Unbalanced Right Parenthesis\n");
				i++;
				flag = 1;
			}
			else if(dfa_error[state.top()][idx]==3) {
			...
			else if(dfa_error[state.top()][idx]==4) {
				state.push(9);
				symbol.push(')');
