...
int prod_ct = 0;
char r1[15][10],gl[15],gr[15][10],temp,templ[15],tempr[15][10],*ptr,temp2[5],dfa[15][15];
int reduce[15];
   ...
   terminal[c_term++] = '$';
            ...
            if(flag == 0) {
                if(reduce[i]==-2 && k == c_term-1) {
                   ...
                   printf("- \t");
                    ...
                    char term = terminal[k];
                    char non_term = gl[reduce[i]];
                    int z;
                    
					for(z = 0; z<n_ct;z++) {
                    	if(non_term_int[z]==non_term) break;
					}
					
					bool is_in = map_term_fol[z].find(term) != map_term_fol[z].end();
					
					if(is_in ) {
						printf("r%d\t",reduce[i]);
					}
					else printf("- \t",non_term);
					//
                }
                else {
                    printf("- \t");
    ...
    while(!(str[i]=='$'&&state.top()==1)) {
        ...
        if(!isupper(str[i]) && dfa1[cur_st][j]!=-1) {
        	...
        	char term = str[i];
            char non_term = gl[reduce[cur_st]];
            int z;
