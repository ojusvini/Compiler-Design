...
int main() {
   ...
   terminal[c_term++] = '$';
	...
	for(j=0;j<c_nterm;j++){
		int idx1 = char_to_int[non_terminal[j]];
		bool is_in = map_term_first[idx1].find('#') != map_term_first[idx1].end();
		if(	is_in && epsilon_present[j] == -1) {
			if(prod_count[j]==1) {
				for(int z = 0; z< prod_ct;z++) {
					if(gl[z]==non_terminal[j]) {
						epsilon_present[j] = z;
						break;
					}
				}
	...
	for(i=0;i<c_nterm;i++) {
		for(j=0;j<c_term;j++) {
			if(terminal[j]!='#'&& parsing_table[i][j]!=-1){
				int ct = parsing_table[i][j];
				printf("M[%c , %c] = %c -> %s\n",non_terminal[i],terminal[j],gl[ct],gr[ct]);
			}
